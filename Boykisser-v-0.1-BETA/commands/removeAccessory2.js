...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('removeaccessory2')
    .setDescription('Remove the accessory from slot 2 for one of your kissers.')
  ...
  run: async ({ interaction }) => {
    ...
    const removedAccessory = user.currentAccessory2?.[selectedKisser];
    if (!removedAccessory) {
      return await interaction.reply({
    ...
    const updatedCurrentAccessory2 = { ...user.currentAccessory2 };
    delete updatedCurrentAccessory2[selectedKisser];

    // Update the database with the modified object
    user.currentAccessory2 = updatedCurrentAccessory2;
